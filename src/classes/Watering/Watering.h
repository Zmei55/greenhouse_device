...
class Watering {
  public:
    ...
    /** Получить значение влажная ли почва true/false */
    bool isSoilWet();
     ...
     * DRY - сухая (например: нужно включить полив)
     * WET - мокрая (например: нужно выключить полив)
     * @param moisture enum DRY/WET
  ...
  private:
    ...
    int16_t _waterPressure = 100; // Давление воды (сила работы насоса), мах 225
    uint16_t _soilWetValue = 3700; // Значение датчика влажности почвы, при котором выключается полив
    uint16_t _soilDryValue = 2700; // Значение датчика влажности почвы, при котором включается полив
